...
"""
    ...
    if financial_scores and "profitability_score" in financial_scores:
        output.append(
            f"**📊 수익성 점수: {financial_scores['profitability_score']}/100점 ({financial_scores['profitability_grade']})**"
        )
        output.append("")
    ...
    if financial_scores and "stability_score" in financial_scores:
        output.append(
            f"**🛡️ 안정성 점수: {financial_scores['stability_score']}/100점 ({financial_scores['stability_grade']})**"
        )
        output.append("")
    ...
    if profitability_assessment and stability_assessment:
        ...
        if financial_scores:
            ...
            if overall_score >= 80:
                strengths.append(
                    f"높은 재무 점수 ({overall_score}점)로 우수한 투자 후보"
                )
            elif overall_score < 60:
                concerns.append(
                    f"낮은 재무 점수 ({overall_score}점)로 신중한 검토 필요"
                )
