...
def format_financial_analysis(analysis_data: Dict) -> str:
    ...
    if profile:
        ...
        if "MarketCapitalization" in profile:
            output.append(
                f"- Market Cap: {analysis_data.get('analysis', {}).get('market_cap', 'No data')}"
            )
    ...
    if profitability_assessment and stability_assessment:
        ...
        if "roe_evaluation" in analysis and "Excellent" in analysis["roe_evaluation"]:
            strengths.append(
                "Strong return on equity indicating efficient use of shareholder capital"
            )

        if (
            "operating_margin_evaluation" in analysis
            and "Elite" in analysis["operating_margin_evaluation"]
        ):
            strengths.append(
                "Industry-leading operating margins demonstrating operational excellence"
            )

        if "debt_evaluation" in analysis:
            if (
                "Very high" in analysis["debt_evaluation"]
                or "High" in analysis["debt_evaluation"]
            ):
                concerns.append(
                    "High leverage that may increase financial risk during economic downturns"
                )
            elif (
                ...
                strengths.append(
                    "Conservative debt levels providing financial flexibility"
                )

        if (
            "current_ratio_evaluation" in analysis
            and "risk" in analysis["current_ratio_evaluation"].lower()
        ):
            concerns.append(
                "Liquidity concerns that may impact short-term financial obligations"
            )
        ...
        if "fcf_margin_evaluation" in analysis:
            if (
                ...
                strengths.append(
                    "Excellent cash generation capabilities supporting future growth and shareholder returns"
                )
            elif "Negative" in analysis["fcf_margin_evaluation"]:
                concerns.append(
                    "Negative free cash flow indicating potential cash burn issues"
                )
