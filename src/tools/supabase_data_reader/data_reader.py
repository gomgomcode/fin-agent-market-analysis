...
class SupabaseDataReader:
    ...
    def get_stock_prices(self, ticker: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """주가 데이터 조회"""
        try:
    ...
    def get_financial_metrics(self, ticker: str, start_date: str, end_date: str, period: Optional[str] = None, limit_count: Optional[int] = None) -> List[Dict[str, Any]]:
        """재무 지표 조회. end_date 이전 최신 데이터를 가져오려면 start_date를 과거로, limit_count=1, desc=True로 설정."""
        try:
            query = self.client.table("financial_metrics") \
                .select("*") \
                .eq("ticker", ticker) \
                .lte("report_period", end_date) # end_date 이전 데이터
            
            if start_date and start_date != "1970-01-01": # start_date가 유효한 경우 필터링 (1970-01-01은 모든 과거를 의미)
            ...
            # 디버깅: 실제 조회된 데이터 출력
            print(f"DEBUG: Financial metrics query for {ticker} (end_date: {end_date}, period: {period}, limit: {limit_count})")
    ...
    def get_company_news(self, ticker: str, start_date: str, end_date: str, limit: int = 100) -> List[Dict[str, Any]]:
        """회사 뉴스 조회"""
        try:
            response = self.client.table("company_news") \
                .select("*") \
                .eq("ticker", ticker) \
                .gte("date", start_date) \
                .lte("date", end_date) \
    ...
    def get_insider_trades(self, ticker: str, start_date: str, end_date: str) -> List[Dict[str, Any]]:
        """내부자 거래 조회"""
        try:
            response = self.client.table("insider_trades") \
                .select("*") \
                .eq("ticker", ticker) \
    ...
    def get_company_facts(self, ticker: str) -> Dict[str, Any]:
        """회사 기본 정보 조회"""
        try:
            response = self.client.table("company_facts") \
                .select("*") \
                .eq("ticker", ticker) \
                .execute()
    ...
    def get_weekly_data_summary(self, ticker: str, end_date: str) -> Dict[str, Any]:
        ...
        start_date_prices_news_insider = (datetime.strptime(end_date, "%Y-%m-%d") - timedelta(days=7)).strftime("%Y-%m-%d")
        ...
        if not latest_financials:
            print(f"DEBUG: No financial data before {end_date}, searching for nearest future data...")
            latest_financials = self._get_nearest_financial_data(ticker, end_date)
        
        if latest_financials and isinstance(latest_financials, list) and len(latest_financials) > 0:
            financial_metrics_data = latest_financials[0]
            financial_metrics_report_date = financial_metrics_data.get("report_period", "N/A")
            # 디버깅: 재무 데이터 확인
            print(f"DEBUG: Found financial data for {ticker}: report_period={financial_metrics_report_date}, period={financial_metrics_data.get('period', 'N/A')}")
            print(f"DEBUG: Sample financial metrics: market_cap={financial_metrics_data.get('market_cap', 'N/A')}, pe_ratio={financial_metrics_data.get('price_to_earnings_ratio', 'N/A')}")
        else:
        ...
        return {
            ...
            "prices": self.get_stock_prices(ticker, start_date_prices_news_insider, end_date),
            "news": self.get_company_news(ticker, start_date_prices_news_insider, end_date, limit=20),
            "insider_trades": self.get_insider_trades(ticker, start_date_prices_news_insider, end_date),
    ...
    def _get_financial_data_before_date(self, ticker: str, end_date: str) -> List[Dict[str, Any]]:
        """해당 날짜 이전의 재무 데이터 조회 (백테스팅 원칙)"""
        # TTM -> Quarterly -> Annual -> All periods 순으로 조회
        for period in ["ttm", "quarterly", "annual", None]:
            financials = self.get_financial_metrics(ticker, "1970-01-01", end_date, period=period, limit_count=3)
            if financials:
                return financials
        return []
    
    def _get_nearest_financial_data(self, ticker: str, reference_date: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.data:
                print(f"DEBUG: Found {len(response.data)} future financial records for {ticker} after {reference_date}")
    ...
    def get_monthly_data_summary(self, ticker: str, end_date: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "financial_metrics": self.get_financial_metrics(ticker, start_date, end_date),
