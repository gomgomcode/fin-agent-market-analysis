...
class BaseWeeklyReporter(ABC):
    ...
    def generate_weekly_report(self, ticker: str, report_date: str) -> str:
        ...
        print(
            f"DEBUG: financial_metrics type: {type(weekly_data.get('financial_metrics'))}"
        )
        print(
            f"DEBUG: financial_metrics content: {weekly_data.get('financial_metrics')}"
        )
        print(
            f"DEBUG: report_date_for_financials: {weekly_data.get('report_date_for_financials')}"
        )
        ...
        print(
            f"DEBUG: insider_analysis for {ticker} ({report_date}): {insider_analysis}"
        )
        ...
        report_params = {
            ...
            "report_date_original": weekly_data.get(
                "report_date_original", report_date
            ),  # 보고서 요청된 기준일 (주로 주의 마지막 날)
        ...
        print(
            f"DEBUG: report_params financial_metrics_data: {report_params['financial_metrics_data']}"
        )
        print(
            f"DEBUG: report_params financial_metrics_report_date: {report_params['financial_metrics_report_date']}"
        )
    ...
    def _analyze_prices(self, prices: List[Dict]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(prices) > 1:
                for i in range(1, len(prices)):
                    ...
                    if (
                        ...
                        and isinstance(curr_close, (int, float))
                        and prev_close != 0
                    ):
            ...
            if daily_returns:
                ...
                volatility = round(
                    statistics.stdev(daily_returns) * 100, 2
                )  # 백분율로 표시
    ...
    def _analyze_news(self, news: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "sentiment_ratio": {
                "positive": round(len(positive_news) / len(news) * 100, 1)
                if news
                else 0,
                "negative": round(len(negative_news) / len(news) * 100, 1)
                if news
                else 0,
