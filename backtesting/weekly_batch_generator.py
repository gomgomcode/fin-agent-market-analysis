...
def generate_complete_weekly_dates(
...
def validate_week_completeness(
    start_date: str, end_date: str, monday_date: str, sunday_date: str
) -> bool:
    """주간 데이터의 완전성 검증"""
...
def check_existing_reports(
    reporter, ticker: str, weeks: List[Tuple[str, str, str]]
) -> List[Tuple[str, str, str]]:
    ...
    try:
        ...
        # 기존 보고서 조회
        response = (
            reader.client.table("stock_reports")
            .select("report_date")
            .eq("ticker", ticker)
        ...
        existing_dates = (
            {report["report_date"] for report in response.data}
            if response.data
            else set()
        )
        ...
        print(
            f"{ticker}: 기존 보고서 {len(existing_dates)}개, 생성할 보고서 {len(missing_weeks)}개"
        )
...
def validate_week_data_availability(
    ticker: str, monday_date: str, sunday_date: str
) -> bool:
    ...
    try:
        ...
        # 해당 주차의 주가 데이터 확인
        response = (
            reader.client.table("stock_prices")
            .select("time")
            .eq("ticker", ticker)
        ...
        if not has_sufficient_data:
            print(
                f"  ⚠️  {monday_date}~{sunday_date}: 데이터 부족 ({price_data_count}일)"
            )
...
def main():
    ...
    if all_weeks:
        ...
        for i, (monday, sunday, report_date) in enumerate(all_weeks[:3], 1):
            ...
            print(
                f"  {i}주차: {monday} ({monday_dt.strftime('%a')}) ~ {sunday} ({sunday_dt.strftime('%a')}) → 보고서일: {report_date}"
            )
    ...
    for ticker in TICKERS:
        ...
        for i, (monday, sunday, report_date) in enumerate(missing_weeks, 1):
            try:
                ...
                sunday_dt = datetime.strptime(sunday, "%Y-%m-%d")
                print(
                    f"[{i}/{len(missing_weeks)}] {ticker} - {monday}({monday_dt.strftime('%a')})~{sunday}({sunday_dt.strftime('%a')}) (보고서일: {report_date})"
                )
    ...
    print(
        f"  - 성공률: {(total_reports_generated / total_reports_to_generate) * 100:.1f}%"
        if total_reports_to_generate > 0
        else "  - 성공률: N/A"
    )
    ...
    if all_weeks:
        ...
        print(
            f"  - 마지막 완전한 주: {all_weeks[-1][0]} (월) ~ {all_weeks[-1][1]} (일)"
        )
    ...
    try:
        ...
        if response.data:
            ...
            for report in response.data:
                ...
                print(
                    f"  - {report['ticker']}: {report['report_date']} ({monday.strftime('%Y-%m-%d')}~{report['report_date']} 주차)"
                )
...
def test_week_generation():
    ...
    for start, end in test_cases:
        ...
        for i, (monday, sunday, report_date) in enumerate(weeks, 1):
            ...
            sunday_dt = datetime.strptime(sunday, "%Y-%m-%d")
            print(
                f"  {i}주차: {monday} ({monday_dt.strftime('%a')}) ~ {sunday} ({sunday_dt.strftime('%a')}) → 보고서: {report_date}"
            )
