...
def create_tables():
    ...
    for i, sql in enumerate(table_sqls, 1):
        try:
            ...
            table_name = sql.split('CREATE TABLE IF NOT EXISTS ')[1].split(' (')[0] if 'CREATE TABLE' in sql else f"table_{i}"
    ...
    for i, sql in enumerate(index_sqls, 1):
        try:
            ...
            index_name = sql.split('CREATE INDEX IF NOT EXISTS ')[1].split(' ON')[0] if 'CREATE INDEX' in sql else f"index_{i}"
