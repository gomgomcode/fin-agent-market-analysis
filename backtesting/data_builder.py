...
def calculate_missing_financial_ratios(financial_data: dict) -> dict:
    ...
    if calculated_data.get('period') == 'quarterly':
        ...
        missing_fields = [field for field, name in null_fields.items() 
        ...
        if missing_fields and ticker and report_period:
            try:
                ...
                if 'interest_coverage' in missing_fields:
                    ...
                    if (net_margin and net_margin > 0) or (operating_margin and operating_margin > 0):
                        # 종목별 추정값 사용
                        ic_estimates = {
                            "AAPL": 150.0, "MSFT": 120.0, "GOOGL": 100.0, 
                            "NVDA": 80.0, "TSLA": 50.0
                        }
                    ...
                    missing_fields = [f for f in missing_fields if f != 'interest_coverage']
                    ...
                    if annual_data:
                        for field in missing_fields:
                            ...
                            if annual_value is not None:
                                ...
                                print(f"  보완: {null_fields[field]} = {annual_value:.4f} (annual 데이터 사용)")
...
def get_nearest_annual_data(ticker: str, quarterly_report_period: str) -> dict:
    ...
    try:
        ...
        if ticker == "AAPL":
            ...
            # 일반적인 경우 더 넓은 범위로 확장
            target_years = [quarterly_date.year, quarterly_date.year - 1, quarterly_date.year - 2]
...
def fetch_and_upsert_financial_metrics_enhanced(supabase_client: Client, table_name: str, ticker: str, overall_start_date: str, overall_end_date: str):
...
if __name__ == "__main__":
    ...
    for ticker in TICKERS:
        ...
        try:
            ...
            fetch_and_upsert_financial_metrics_enhanced(supabase_client, SUPABASE_FINANCIALS_TABLE, ticker, FULL_START_DATE, FULL_END_DATE)
            time.sleep(1)
